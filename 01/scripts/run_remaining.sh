#!/bin/bash
# Complete the remaining scaling + extended benchmark work
set -e
CH="clickhouse-client --database exp01_compression"
OUTDIR="/tmp/exp01_extended"
mkdir -p $OUTDIR

echo "=== Creating remaining 500m variant tables ==="

# v3_percolumn
echo "Creating scale_500m_v3_percolumn..."
$CH --query "DROP TABLE IF EXISTS scale_500m_v3_percolumn"
$CH --query "CREATE TABLE scale_500m_v3_percolumn (
...
$CH --query "INSERT INTO scale_500m_v3_percolumn SELECT * FROM source_500m"
echo "v3 loaded, optimizing..."
$CH --query "OPTIMIZE TABLE scale_500m_v3_percolumn FINAL"
echo "v3 done"

# v4_percolumn_zstd
echo "Creating scale_500m_v4_percolumn_zstd..."
$CH --query "DROP TABLE IF EXISTS scale_500m_v4_percolumn_zstd"
$CH --query "CREATE TABLE scale_500m_v4_percolumn_zstd (
...
$CH --query "INSERT INTO scale_500m_v4_percolumn_zstd SELECT * FROM source_500m"
echo "v4 loaded, optimizing..."
$CH --query "OPTIMIZE TABLE scale_500m_v4_percolumn_zstd FINAL"
echo "v4 done"

# v5_aggressive
echo "Creating scale_500m_v5_aggressive..."
$CH --query "DROP TABLE IF EXISTS scale_500m_v5_aggressive"
$CH --query "CREATE TABLE scale_500m_v5_aggressive (
...
$CH --query "INSERT INTO scale_500m_v5_aggressive SELECT * FROM source_500m"
echo "v5 loaded, optimizing..."
$CH --query "OPTIMIZE TABLE scale_500m_v5_aggressive FINAL"
echo "v5 done"

echo "=== All 500m variant tables created ==="
    ...
    for v in $VARIANTS; do
        ...
        for run in 1 2 3 4 5; do
            for q in Q1 Q2 Q3 Q4 Q5 Q6; do
                ...
                QID="sc_cold_${q}_${size}_${v}_${run}_$(date +%s%N)"
                $CH --query_id "$QID" --query "$SQL" 2>/dev/null
                sleep 0.2

                # Warm
                QID2="sc_warm_${q}_${size}_${v}_${run}_$(date +%s%N)"
...
$CH --query "
SELECT
    extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][2] AS query,
    extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][3] AS size,
    extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][4] AS variant,
    extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][5] AS run,
    extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][1] AS temp,
    query_duration_ms,
    read_rows,
    read_bytes,
    ProfileEvents['OSCPUVirtualTimeMicroseconds'] AS cpu_us
FROM system.query_log
WHERE query_id LIKE 'sc_%' AND type = 'QueryFinish'
    AND extractAllGroupsVertical(query_id, 'sc_(cold|warm)_(Q[0-9])_([0-9]+m)_([a-z0-9_]+)_([0-9]+)_')[1][1] != ''
...
# ═══════════════════════════════════════════════════
# SCALING INGEST BENCHMARK (3 runs per size × variant)
# ═══════════════════════════════════════════════════
...
# 500m ingest - only 1 run per variant (too slow for 3)
size=500m; ROWS=500000000; src="source_500m"
for v in $VARIANTS; do
    INGEST_TBL="ingest_tmp_${v}"
    $CH --query "DROP TABLE IF EXISTS $INGEST_TBL"
    create_variant_table "$INGEST_TBL" "$v"
